...
public class Notas {
    ...
    public void agregarNota(ArrayList<Usuario> usuariosCreados) {
        ...
        notasList = nota.insertarNotas(usuario);
    ...
    public  ArrayList insertarNotas(Usuario usuario) {
        int idusuario, idCarrera, idMateria;
        int cantidadNotas;
        double valor=0d;
        double porcentaje=0, sumaPorcentaje=0;
        ArrayList<Notas> notasList = new ArrayList<>();
        ArrayList<Notas> nuevaNota = new ArrayList<>();
        ArrayList<Usuario> usuario1= new ArrayList<>();

        System.out.println("Cuantas notas desea ingresar a la materia:" + usuario.getPrimerNombre()+usuario.getMateria()+":");
        cantidadNotas = entrada.nextInt();
        for (int i = 0; i < cantidadNotas; i++) {
            sumaPorcentaje = usuario.validarNotas2(notasList);
